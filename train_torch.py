...
# })
def train(train_data_loader,config,logger):
    """
    整网从头训练
    args:
    train_data_loader:训练数据集迭代器
    config:训练配置
    logger:日志生成器
    """
    ...
    # backbone.load_state_dict(torch.load(config.backbone,map_location=device))
    ...
    for epoch in range(config.epochs):
        logger.info("Training without pretained Model,epoch:{}".format(epoch).center(60,'-'))
        # net.train()
        ...
        if avg_loss<best_loss:#训练平均损失下降时，保存模型参数
            best_model_weights=copy.deepcopy(net.state_dict())
            # best_model_weights=copy.deepcopy(dict(backbone.state_dict().items()+head.state_dict().items()))
            best_loss=avg_loss 
        ...
        if epoch%config.save_ckpt_epochs==0:#间隔一定轮数保存模型参数
            torch.save(best_model_weights,os.path.join(config.save_ckpt_path,str(epoch)+'.pth'))
        logger.info("Training Time {:.6f} s | Avg Testing Loss {:.6f} | Avg Accuracy Rate {:.6f}".format(time.time()-start_time, avg_loss,correct/total))
    print('Finish Training')

def pre_train(train_data_loader,config,logger):
    """
    backbone加载预训练模型，整网微调
    args:
    train_data_loader:训练数据集迭代器
    config:训练配置
    logger:日志生成器
    ...
    scheduler=optim.lr_scheduler.ReduceLROnPlateau(optimizer,
                                                    ...
                                                    patience=3)
    criterion=nn.CrossEntropyLoss()
    # net.load_state_dict(torch.load(config.pretrained_ckpt,map_location=device))
    best_loss=1e9
    best_model_weights=copy.deepcopy(net.state_dict())
    # best_model_weights=copy.deepcopy(dict(backbone.state_dict().items()+head.state_dict().items()))
    # backbone.load_state_dict(torch.load(config.backbone,map_location=device))
    ...
    net.train()
    for epoch in range(config.epochs):
        logger.info("Training with pretained Model,epoch:{}".format(epoch).center(60,'-'))
        # net.train()
        start_time=time.time()
        total=0
        correct=0
...
def pre_train_full(train_data_loader,config,logger):
    """
    加载已训练过的模型，整网微调
    args:
    train_data_loader:训练数据集迭代器
    ...
    net.train()
    for epoch in range(config.epochs):
        logger.info("Fine-tune without pretained Model,epoch:{}".format(epoch).center(60,'-'))
        # net.train()
        start_time=time.time()
        total=0
        correct=0
        ...
        if writer:
            ...
            writer.add_scalars('training_accuracy', {'train': correct/total}, epoch+1)
        if epoch%config.save_ckpt_epochs==0:
            torch.save(best_model_weights,os.path.join(config.save_ckpt_path,'pre'+str(epoch)+'.pth'))
        logger.info("Training Time {:.6f} s | Avg Testing Loss {:.6f} | Avg Accuracy Rate {:.6f}".format(time.time()-start_time, avg_loss,correct/total))
    print('Finish Training')
...
def fine_tune(train_data_loader,config,logger):
    """
    迁移学习模型的优化
    args:
    train_data_loader:训练数据集迭代器
    ...
    net.train()
    for epoch in range(config.epochs):
        logger.info("Fine-tune with pretained Model,epoch:{}".format(epoch).center(60,'-'))
        # net.train()
        start_time=time.time()
        total=0
        correct=0
...
if __name__=='__main__':
    ...
    logging.basicConfig(level=logging.INFO,format="%(message)s",handlers=[logging.FileHandler(os.path.join(config.TRAIN_LOG_PATH, "{:%Y-%m-%d_%H-%M-%S}.log".format(datetime.now()))), logging.StreamHandler()])
    logger=logging.getLogger()
    data_loader=create_dataset(config)
    # train(data_loader,config,logger)
    # pre_train_full(data_loader,config,logger)
    # pre_train(data_loader,config,logger)
    fine_tune(data_loader,config,logger)
    print("Finish all!!!")
    pass